...
import styles from './ProjectsTile3.module.scss';
...
function ProjectsTile3() {
  ...
  return (
    <section className={styles.ProjectsTile3}>
      <div className={styles.ProjectsTile3__container}>
        <div className={`${styles.ProjectsTile3__layer} ${styles.ProjectsTile3__layer_bot}`}>
          <div ref={animationRef} className={`${styles.ProjectsTile3__items} ${styles.animate}`}>
            {items.map(({ id, picture, src, title }) => (
              <div className={styles.ProjectsTile3__item} style={{ backgroundImage: `url(${picture})` }} key={id}>
                <a className={`${styles.ProjectsTile3__link} ${styles.ProjectsTile3__link_bot}`} href={src}>
                  <div className={styles.ProjectsTile3__text}>
                    <div className={styles.ProjectsTile3__num}>{String(id).padStart(2, '0')}</div>
                    <div className={styles.ProjectsTile3__title} dangerouslySetInnerHTML={{ __html: title }}></div>
                  </div>
                </a>
              </div>
            ))}
        ...
        <div
          className={`${styles.ProjectsTile3__images} ${
            hoveredIndex !== null ? `${styles[`active${hoveredIndex + 1}`]}` : dishoveredIndex !== null ? `${styles[`disactive${dishoveredIndex + 1}`]}` : ''
          }`}
        >
          {items.map(({ id }, index) => (
            <div className={styles.ProjectsTile3__image} key={id}></div>
          ))}
        </div>

        <div className={`${styles.ProjectsTile3__layer} ${styles.ProjectsTile3__layer_top}`}>
          <div className={`${styles.ProjectsTile3__items}`}>
            {items.map(({ id, src, title }, index) => (
              <div
                className={styles.ProjectsTile3__item}
                key={id}
                onMouseEnter={() => handleMouseEnter(index)}
                onMouseLeave={() => handleMouseLeave(index)} // изменяем этот обработчик
              >
                <a className={styles.ProjectsTile3__link} href={src}>
                  <div className={styles.ProjectsTile3__text}>
                    <div className={styles.ProjectsTile3__num}>{String(id).padStart(2, '0')}</div>
                    <div className={styles.ProjectsTile3__title} dangerouslySetInnerHTML={{ __html: title }}></div>
                  </div>
